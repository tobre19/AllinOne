...
cv::Point2f Camera::getTargetLocation(cv::Mat table)
{
    ...
    cv::Mat1b redmask;
    cv::inRange(hsv, cv::Scalar(0, 150, 150), cv::Scalar(10, 255, 255), redmask); //target

    std::vector<cv::Vec3f> circles;
    cv::HoughCircles(redmask, circles, CV_HOUGH_GRADIENT, 2, 10, 80, 40, 15, 20);
    ...
    for( size_t i = 0; i < circles.size(); i++ )
    {
        ...
        // circle center
        cv::circle( table, target, 1, cv::Scalar(0,100,100), 3, cv::LINE_AA);
        // circle outline
        int radius = c[2];
        cv::circle( table, target, radius, cv::Scalar(255,0,255), 3, cv::LINE_AA);
    }
...
void Camera::drawTarget(cv::Mat table)
{
    cv::Point2f target;
    cv::Mat hsv;
    cv::Mat gaussian;
